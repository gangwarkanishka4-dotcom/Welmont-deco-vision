...
import type { Camera, CameraCreateInput, CameraUpdateInput, Classroom } from '../../types';
...
function emptyForm(classrooms: Classroom[]): FormState {
  return {
    name: '',
...
function formFromCamera(camera: Camera): FormState {
  return {
    name: camera.name,
...
export function CameraFormModal({
  ...
  const [form, setForm] = useState<FormState>(() => (camera ? formFromCamera(camera) : emptyForm(classrooms)));
  ...
  const [lastCameraId, setLastCameraId] = useState<string | null | undefined>(undefined);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (camera) {
        const update: CameraUpdateInput = { ...base };
        if (form.rtsp_password) update.rtsp_password = form.rtsp_password;
        await api.updateCamera(camera.id, update);
      } else {
        const create: CameraCreateInput = { ...base, rtsp_password: form.rtsp_password };
        await api.createCamera(create);
      }
      onSaved();
    } catch (err) {
    ...
    <Modal open={open} onClose={onClose} title={camera ? 'Edit Camera' : 'Add Camera'} widthClassName="max-w-2xl">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <input className="input w-full" value={form.rtsp_host} onChange={(e) => set('rtsp_host', e.target.value)} placeholder="192.168.1.50" />
        </div>
        <div>
        ...
        <div>
          ...
          <input className="input w-full" value={form.rtsp_path} onChange={(e) => set('rtsp_path', e.target.value)} placeholder="/stream1" />
        </div>
        <div>
          <label className="label">Username</label>
          <input className="input w-full" value={form.rtsp_username} onChange={(e) => set('rtsp_username', e.target.value)} />
        </div>
        <div>
          <label className="label">Password {camera && <span className="text-slate-600">(leave blank to keep current)</span>}</label>
          <input
            type="password"
            className="input w-full"
        ...
        <div>
          ...
          <input className="input w-full" value={form.resolution_width} onChange={(e) => set('resolution_width', e.target.value)} />
        </div>
        <div>
          <label className="label">Resolution Height</label>
          <input className="input w-full" value={form.resolution_height} onChange={(e) => set('resolution_height', e.target.value)} />
        </div>
      </div>
