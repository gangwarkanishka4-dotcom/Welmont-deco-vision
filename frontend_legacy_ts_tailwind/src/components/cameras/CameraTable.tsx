...
import { CameraStatusDot } from '../badges/CameraStatusDot';
...
export function CameraTable({ cameras, classroomsById, onEdit, onConfigure, onDeleted, onToggled }: Props) {
  const [testState, setTestState] = useState<Record<string, { testing: boolean; message?: string; success?: boolean }>>({});
  const [busyId, setBusyId] = useState<string | null>(null);

  async function handleTest(cam: Camera) {
  ...
  async function handleToggleEnabled(cam: Camera) {
    setBusyId(cam.id);
    try {
