...
import type { Camera, CalibrationPoint, RoiPoint } from '../../types';

type Mode = 'roi' | 'calibration';

export function CameraConfigModal({ open, onClose, camera }: { open: boolean; onClose: () => void; camera: Camera }) {
  ...
  const [pendingPixelY, setPendingPixelY] = useState<number | null>(null);
  ...
  const toDisplay = (native: { x: number; y: number }) => {
  ...
  const toNative = (display: { x: number; y: number }) => {
    const el = canvasRef.current;
