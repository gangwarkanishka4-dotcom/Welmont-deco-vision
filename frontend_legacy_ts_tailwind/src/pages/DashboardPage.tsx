...
import type { ClassroomStatusEntry } from '../types';

export function DashboardPage() {
  const { classrooms, loading: directoryLoading } = useDirectory();
  const [entries, setEntries] = useState<Map<string, ClassroomStatusEntry>>(new Map());
  ...
  useEffect(() => {
    ...
    (async () => {
      const results = await Promise.all(
        classrooms.map((c) => api.getClassroomStatus(c.id).catch(() => [] as ClassroomStatusEntry[])),
      );
      if (cancelled) return;
      const next = new Map<string, ClassroomStatusEntry>();
